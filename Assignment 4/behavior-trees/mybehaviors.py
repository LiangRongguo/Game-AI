...
def treeSpec(agent):
	...
	spec = [
		(Selector, 0),
			(Retreat, 0.5, 1),
			[(HitpointDaemon, 0.5, 2),
				[(Selector, 3),
					[(MyDaemon, 4),
						[(Sequence, 5),
							(ChaseMinion, 6), (KillMinion, 7)]
					],
					[(Sequence, 8),
					 	(ChaseHero, 9), (KillHero, 10)]
				 	]
			 ]
	]
...
class KillMinion(BTNode):
	...
	def dodgeBullet(self):
		# print("here")
		if self.agent.canDodge():
			bullets = []
			for b in self.agent.getVisibleType(Bullet):
				if b.owner.getTeam() != self.agent.getTeam():
					bullets.append(b)
			...
			if len(bullets) > 0:
				...
				dist = 0
				for b in bullets:
					d = distance(b.getLocation(), self.agent.getLocation())
					if best == None or d < dist:
						best = b
						dist = d

				vector = (math.cos(math.radians(best.orientation + 90)), -math.sin(math.radians(best.orientation + 90)))
				(x, y) = tuple(map(lambda x, y: x + y, self.agent.getLocation(), (
				vector[0] * self.agent.getMaxRadius() * 3, vector[1] * self.agent.getMaxRadius() * 3)))
				if not insideObstacle((x, y), self.agent.world.getObstacles()):
					self.agent.dodge(best.orientation + 90)
				else:
					self.agent.dodge(best.orientation - 90)
	...
	def execute(self, delta=0):
		...
		for e in enemies:
			if isinstance(e, Hero):
				hero = e
				break
		if hero is None or self.agent.level <= hero.level + self.advantage:
			# fail check
			print("exec", self.id, "fail")
			return False
		else:
...
class MyDaemon(BTNode):
	...
	def execute(self, delta = 0):
		...
		for enemy in enemies:
			if distance(self.agent.getLocation(), enemy.getLocation()) < (self.agent.getRadius()*AREAEFFECTRANGE) + (enemy.getRadius()):
				self.agent.areaEffect()

		if hero is not None and distance(self.agent.getLocation(), hero.getLocation()) < 300:
			# fail check
			return False
		else:
			# Check didn't fail, return child's status
